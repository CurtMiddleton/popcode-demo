...
export const COMPANION_CARD = {
  ...
  //
  // UNVERIFIED — pulled from Prodigi's product page, not the API. Confirm with:
  //   PRODIGI_API_KEY=... node scripts/verify-prodigi-sku.mjs GLOBAL-POST-MOH-6X4-BLA
  // Safe to carry until then: a wrong SKU surfaces instantly as SkuNotFound at
  // quote time, with no charge and nothing printed.
  ...
  // Geometry the renderer produces, taken from the approved artwork's own
  // TrimBox/BleedBox. Re-check against the SKU's stated dimensions — Prodigi
  // lists this size as 152 × 102 mm.
  trimIn: { w: 6, h: 4 },
  bleedIn: 0.125,
  dpi: 300,

  // Face -> Prodigi print area. THIS IS THE ONE THING THAT CHANGES once the
  // SKU is verified:
  //   one print area  -> leave as is (front only; the reverse stays unprinted)
  //   two print areas -> add { face: 'back', printArea: '<name>' } and rename
  //                      'default' to whatever the SKU calls the front
  // The card is designed with a BLANK back, so the back asset is a plain white
  // face. Sending it only matters when the SKU makes the area required —
  // otherwise we send nothing and no white ink is laid down.
  // Supplying fewer assets than the SKU declares REQUIRED fails the order with
  // MissingRequiredAssets — the trap this repo hit on its first Prodigi
  // integration. buildPostcardAssets() renders both faces either way, so this
  // stays a config change rather than a build.
  faces: [{ face: 'front', printArea: 'default' }],
};
...
/**
 * The distinct Popcode slugs in a cart that earn a card.
 *
 * ONE CARD PER SLUG, not per order and not per item. The brief leaned toward a
 * single card listing every link, but the approved artwork carries exactly one
 ...
 *
 * @param {Array<{productType: string, slug?: string}>} lines
 * @returns {string[]} distinct slugs, in cart order
 */
export function companionCardSlugs(lines) {
  const seen = new Set();
  for (const l of lines || []) {
    if (!COMPANION_CARD_FOR.has(l.productType)) continue;
    if (l.slug) seen.add(l.slug);
  }
  return [...seen];
}
