...
export const PRODUCTS = {
  ...
  // 11oz coloured photo mugs. The colour is baked into the SKU rather than
  // carried as an attribute, which is the same shape as the frame colours on
  // `framed`/`tile` — black keeps the bare id so the UI's colour mechanism and
  // any saved design resolve the same way.
  //
  // ONE print area, a 190×100 mm wrap all the way round (aspect 1.9), so the
  // photo is a panorama rather than a front panel and there is nowhere to put
  // the URL except in the artwork. `wrap` is what tells the builder to set it
  // vertically at the trailing edge, beside the handle.
  //
  // Prodigi lists nine colours (black, orange, yellow, lime green, sky blue,
  // purple, pink, red, white) but publishes SKUs for only these five. The other
  // four are NOT guessed — a guessed SKU is how this catalogue has been broken
  // before. Add them once `verify-prodigi-sku.mjs` confirms the real codes.
  //
  // ⚠ THE WRAP SIZE IS THE ONE THING STILL UNCONFIRMED. Prodigi's catalogue is
  // inconsistent about it: H-MUG-11OZ-BLA reports 19x10 cm (the wrap, and what
  // the product page's table says), while the four coloured ones report
  // 85x90 mm — which is the mug's own body, not a print area. The per-print-area
  // dimensions come back empty for all five, so the API cannot settle it. These
  // are the same physical 11oz mug, so 190x100 is used throughout.
  // Confirm against Prodigi's downloadable "11oz coloured photo mug template"
  // before the first real order; if it disagrees, this number and the matching
  // one in order.html's mirror are the only two places to change.
  mug: [
    ['BLA',  'Black'],
    ['RED',  'Red'],
    ['YLW',  'Yellow'],
    ...
    id: `mug-11oz${i === 0 ? '' : '-' + code.toLowerCase()}`,
    label: `11oz Photo Mug (${name})`,
    sku: `H-MUG-11OZ-${code}`,
    aspect: 190 / 100,
