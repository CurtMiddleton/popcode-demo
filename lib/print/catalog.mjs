...
export const PRODUCTS = {
  print: [
    // Small sizes. Prodigi publishes the enhanced-matte-art range as 4x6" and
    // up, so these sit inside it — but the exact SKU TOKENS are UNVERIFIED:
    // there was no API key in the environment they were written in. Verify
    // before they reach a customer:
    //   PRODIGI_API_KEY=test_... node scripts/verify-prodigi-sku.mjs \
    //     GLOBAL-FAP-4x6 GLOBAL-FAP-5x7 GLOBAL-FAP-6x6 GLOBAL-FAP-8x8
    // A wrong SKU fails at QUOTE time (before any charge), but it reads to the
    // customer as "we can't ship this size" — wrong, and silent to us.
    { id: 'fap-4x6',   label: 'Fine Art Print 4\u00d76"',      sku: 'GLOBAL-FAP-4x6',   aspect: 4 / 6,   sizing: 'fillPrintArea', printArea: 'default', attributes: {} },
  ...
  framed: [
    // Small classic frames. Same UNVERIFIED caveat as the small prints above:
    //   PRODIGI_API_KEY=test_... node scripts/verify-prodigi-sku.mjs \
    //     GLOBAL-CFP-5x7 GLOBAL-CFP-6x6 GLOBAL-CFP-8x8
    ['5x7', 5 / 7],
    ['6x6', 1],
    ['8x8', 1],
    ['8x10', 8 / 10],
...
export const COMPANION_INSERT = {
  // OFF until the A6 artwork has been through a real order. Turning it on
  // cannot break checkout the way the line item could — an insert is not part
  // of the quote — but it does put artwork in front of a customer.
  enabled: false,
...
export function sumQuoteMinor(quoteResponse) {
  ...
  return { totalMinor: itemsMinor + shippingMinor, itemsMinor, shippingMinor, currency };
}
