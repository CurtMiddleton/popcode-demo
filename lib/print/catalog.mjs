...
export const COMPANION_CARD = {
  ...
  // OFF until the SKU above is verified. The companion joins the SAME Prodigi
  // quote as the rest of the cart, so an unresolvable SKU would fail the quote
  // for the WHOLE order — taking checkout down, not just the card. Flipping this
  // to true is the go-live step, after scripts/verify-prodigi-sku.mjs passes.
  enabled: false,
...
export function companionCardPaths(slug) {
  return {
    front: `${slug}/companion-card-front.png`,
    back: `${slug}/companion-card-back.png`,
  };
}
