...
export const PRODUCTS = {
  ...
  book: [
    ...
    // 8.3 × 8.3" (210 × 210 mm) square layflat. HIDDEN — THE SKU BELOW IS WRONG.
    // It was a guess following the BOOK-FE-{size}-{orientation}-{binding}-{finish}
    // pattern of the verified A4 SKU, and the live API refuses to quote it. The
    // size itself is real (Prodigi's own portfolio lists "Square: 8.3 x 8.3"
    // (210 x 210mm)") but no published page anywhere gives the SKU string, and
    // the catalogue endpoint is exact-match only.
    //
    // Until the real string is known this size is off the menu, so nobody can
    // design a book that fails at checkout — which is what happened before:
    // the whole book got built and the order died on the last step.
    // `scripts/find-book-sku.mjs` probes the plausible spellings with a Prodigi
    // key; the dashboard's product catalogue or Prodigi support also has it.
    // When you have it: fix the sku, drop `hidden`, and drop `unavailable` from
    // BOOK_SIZES['square-8x8'] in public/book.html.
    { id: 'book-21sq-layflat', label: 'Layflat Photo Book — 8×8" Square', sku: 'BOOK-FE-21X21-SQ-LF-G', isBook: true, sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minPages: 18, maxPages: 122, hidden: true },
  ],
...
export const COMPANION_INSERT = {
  ...
  //
  // STILL UNCONFIRMED: orientation. We supply LANDSCAPE (148 × 105mm) to match
  // the approved artwork, while Prodigi documents the SKU in portrait notation
  // (105 × 148mm). Read the proof image on the first order that carries a card
  // and flip both this and public/postcard-render.js if it comes out rotated.
  enabled: true,

  // Prodigi's insert postcard on 260gsm ultra smooth, "including 4mm border",
  // pre-cut so there is no bleed to supply. Prodigi states the size in portrait
  // notation (105 × 148mm); we supply it LANDSCAPE to match the approved
  // artwork. Must agree with PRINT.insertPx in public/postcard-render.js, which
  // is what actually produces the file.
  trimMm: { w: 148, h: 105 },
  borderMm: 4,
  dpi: 300,
  pxSize: { w: 1748, h: 1240 },
};
...
export function sumQuoteMinor(quoteResponse) {
  ...
  return { totalMinor: itemsMinor + shippingMinor, itemsMinor, shippingMinor, currency, lineItems };
}
