...
export const PRODUCTS = {
  ...
  /* ── GIFT PRODUCTS (magnets, stickers, ornaments, mugs) ────────────────
     Added 2026-09-22. Four things set these apart from everything above, and
     each one is a trap the flat/wall products never had to think about:

     1. EVERY ONE IS FULFILLED IN THE UK. Prodigi's own product pages say so
        (magnets UK, kiss-cut stickers UK/EU, Christmas UK, mugs UK/SE). The
        flat and wall products are GLOBAL-* and route to a US lab for a US
        address, so a cart mixing the two CANNOT ship as one parcel — it splits
        across the Atlantic, with a second shipping charge AND a second set of
        branded inserts (those are billed per shipment, ~$3.75 a time, and are
        in no quote). This is the same mechanism that made a companion postcard
        add $46 to an order in September. The cart already quotes and displays
        per-parcel shipping, so the customer sees the truth — but the margin
        hit is ours, which is what `minCopies` below is for.

     2. THE ASSET RULES DIFFER PER PRODUCT. Stickers want PNG with 2.5 mm of
        padding and have a MAXIMUM pixel size; mugs want JPEG at 300 DPI. A
        wrong format or an oversized file is rejected AFTER the customer has
        paid, so the spec travels with the variant in `asset` and the builder
        honours it rather than assuming JPEG like every product above.

     3. SOME ARE ROUND. A bauble or ceramic ornament is die-cut from a square
        asset, so the bottom-right corner — where the Popcode badge has always
        gone — is cut away. `round: true` moves the badge inside the inscribed
        circle. Getting this wrong means shipping a scannable print with the
        scan badge sliced off.

     4. A MUG HAS ONE WRAP-AROUND PRINT AREA, NOT A FRONT. There is no back to
        put the URL on, so it goes INTO the artwork: `wrap` marks the product
        as a 360° wrap and tells the builder to set the popcode.app URL
        vertically at the trailing edge, which is the panel that ends up beside
        the handle. See buildWrapArt() in public/order.html.

     ALL 16 SKUs VERIFIED against GET /v4.0/products/{sku} on the LIVE Prodigi
     catalogue, 2026-09-22. Every one resolves, and every one declares exactly
     ONE required 'default' print area — so the single asset this app builds is
     the right shape for all of them and none can fail with
     MissingRequiredAssets. Re-check after any catalogue edit with:
         PRODIGI_API_KEY=… node scripts/verify-prodigi-sku.mjs <SKU>...

     The attributes Prodigi reports for these are SINGLE-VALUED and therefore
     fixed by the SKU itself (mugs: color + size; aluminium ornaments: style) —
     unlike the classic frames, whose `color` comes back as a pipe-separated
     list of eight and so must be chosen and sent. That is why every variant
     below carries `attributes: {}`: there is nothing left to choose, and
     sending a field Prodigi does not expect is its own rejection.
     ─────────────────────────────────────────────────────────────────────── */

  // Fridge magnets. ONLY the single-image SKUs (MAG-1-*). MAG-4-* and MAG-9-*
  // are photo-collage magnets that declare 4 and 9 required print areas, and
  // this app supplies exactly one asset — they would fail with
  // MissingRequiredAssets, so they are deliberately not offered.
  magnet: [
    { id: 'mag-4in', label: 'Photo Magnet 4×4"', sku: 'MAG-1-10X10', aspect: 1, sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minCopies: 3 },
    { id: 'mag-6in', label: 'Photo Magnet 6×6"', sku: 'MAG-1-15X15', aspect: 1, sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minCopies: 3 },
  ],

  /* Kiss-cut vinyl stickers.

     `aspect` is the SHEET, not the smaller die-cut print area. Prodigi's own
     file-requirements table settles it: the accepted pixel sizes for the 3x4"
     sticker run 360x480 to 900x1200, both exactly 3:4, while its print area
     (66.9 x 95.3 mm) is 0.70. Supply the sheet ratio and the die takes the
     sticker out of the middle of it.

     PNG only — Prodigi rejects JPEG for these — at 300 DPI, with 2.5 mm of
     padding round the artwork (30 px at 300 DPI) and, unusually, a MAXIMUM
     pixel size as well as a minimum. A phone photo cropped square is about
     3000 px, so the cap is real and the builder downscales to it; `padPx` keeps
     the Popcode badge out of the margin the die trims.

     All four verified against GET /v4.0/products/{sku} on the LIVE catalogue
     (2026-09-22): one required 'default' print area each, no attributes. */
  sticker: [
    { id: 'sti-3x4',   label: 'Sticker 3×4"',     sku: 'M-STI-3X4',     aspect: 3 / 4, sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minCopies: 3, asset: { format: 'png', padPx: 30, maxPx: { w: 900,  h: 1200 } } },
    { id: 'sti-5_5',   label: 'Sticker 5.5×5.5"', sku: 'M-STI-5_5X5_5', aspect: 1,     sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minCopies: 3, asset: { format: 'png', padPx: 30, maxPx: { w: 1650, h: 1650 } } },
    { id: 'sti-8_5',   label: 'Sticker 8.5×8.5"', sku: 'M-STI-8_5X8_5', aspect: 1,     sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minCopies: 3, asset: { format: 'png', padPx: 30, maxPx: { w: 2550, h: 2550 } } },
    { id: 'sti-14',    label: 'Sticker 14×14"',   sku: 'M-STI-14X14',   aspect: 1,     sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minCopies: 3, asset: { format: 'png', padPx: 30, maxPx: { w: 4200, h: 4200 } } },
  ],

  // Christmas tree ornaments.
  //
  // THE URL CANNOT GO ON THE BACK. Prodigi states "Single-sided print" for the
  // aluminium, ceramic and glass ornaments. The plastic bauble says
  // "Double-sided print", but its own description explains what that means —
  // "your design showcased on both sides for a 360-degree display", i.e. ONE
  // artwork shown twice on a card insert, not a second print area to put
  // something different on. So every ornament here carries the companion card
  // in the box and nothing printed on a reverse. Confirm against the API's
  // printAreas before assuming otherwise.
  //
  // The ceramic ornament is hand-printed on white ceramic: "any white areas
  // within your artwork will not be printed" — white means bare ceramic, so a
  // white-on-transparent mark would simply disappear. The badge is coloured,
  // which is why it survives; do not switch these to a white mark.
  /* Ornaments moved from Prodigi to Printify. Prodigi has no US lab for them —
     3 ceramic ornaments quoted $31.98 goods against $25.65 of UK shipping, on a
     product whose whole point is arriving before Christmas. Printify's blueprint
     1747 is fulfilled by M.i.A Merchandise in the USA.

     It is DOUBLE-SIDED, and that is why it is the one gift product with no
     companion card: Printify has no insert mechanism at all (the board book is
     excluded from COMPANION_INSERT_FOR for the same reason), so the URL has to
     live on the product. The back print area carries it.

     The old Prodigi SKUs are gone rather than kept as a UK fallback: nothing
     routes by region, so a second set of variants would only be reachable by
     picking them, and every saved design would then depend on which. No designs
     exist to break — ornaments have never been sellable (NO_US_FULFILMENT). */
  ornament: [
    {
      id: 'orn-ceramic',
      label: 'Ceramic Ornament 2.9" × 2.9"',
      sku: 'PRINTIFY-ORN-1747-118761',
      aspect: 1,
      round: true,
      minCopies: 1,
      printify: {
        ...
        printProviderId: 80,
        variantId: 118761,
        /* Per-copy product cost. Printify does not expose it in the catalog API
           (the blueprint endpoints carry ids and print areas, not price), so it
           is declared here or quote() refuses to price the product at all.

           $8.01 is M.i.A Merchandise's listed price for this variant in the
           dashboard, and this blueprint has exactly one variant, so it is not a
           "from" across sizes. CONFIRM IT against the line_item cost on the first
           send_to_production:false order — that is where the board book's 1650
           came from. A small error is absorbed by the 1.7x markup; a large one
           would not be. Shipping is NOT here: it is quoted live per address. */
        baseCostMinor: 801,
        // Printify maps assets onto these by name; both are 938x938, the square
        // the round ornament is die-cut from.
        positions: ['front', 'back'],
      },
      asset: { format: 'png', maxPx: { w: 938, h: 938 } },
    },
  ],

  /* 11oz photo mugs.

     GLOBAL-MUG-W, not the coloured mugs, and that is the whole point. Prodigi's
     GLOBAL-* products are routed to the print lab nearest the customer — the
     same mechanism as the GLOBAL-FAP prints — so a US order is made in the US
     and posted domestically, and can share a parcel with the prints instead of
     arriving separately from Britain.

     The coloured mugs (H-MUG-11OZ-*, below, hidden) are made in the UK only. A
     live quote on 2026-09-22 returned NO US shipping option for any of the five,
     and where Prodigi does post small goods from the UK to the US it charged
     $33.18 a parcel — more than ten times the goods on a small item. They are
     kept so that a saved design still resolves and so the option exists if
     Popcode ever sells into the UK or EU, but they are not offered.

     ONE print area, which the API states exactly: a 228.6 x 94.83 mm band all
     the way round (aspect 2.41). Two things follow from that shape, and `wrap`
     is what marks them:

       The artwork DEFAULTS TO FIT, not fill. A 2.41 band is far wider than any
       ordinary photograph, so filling it crops away most of the picture — and a
       customer who never opens Adjust photo would order that without knowing.
       Fit shows the whole photo, centred, and the white it leaves at each end
       is unprinted ceramic falling either side of the handle. That is also
       where the picture belongs: the middle of the band sits opposite the
       handle, the one spot that faces outward whichever hand holds it and is
       never split by the handle itself.

       The badge goes at the foot of the MIDDLE of the band, not in a corner.
       A wrap has no corner anyone sees — its edges finish beside the handle.

     There is NO printed URL. A mug carries the companion card in the box like
     every other Prodigi product here; a line of type running up a photograph
     was the only place a mug could take one, and it was not worth the picture.
     (The coloured mugs never published a usable print area at all, which is why
     their wrap figure below is the product page's and not the API's; moot while
     they are hidden.)

     Verified 2026-09-22: one required 'default' print area, and its attributes
     (color: white, size: 313ml/11oz) are single-valued, so fixed by the SKU.
     Quoted the same day to a US address: $10.00 goods + $6.45 shipping, against
     $33.18 for the UK-made mugs — the whole reason this is the SKU we sell.

     Prodigi ships this one to the UK, EU and US only. A mug to anywhere else
     comes back as an empty quote, which the shop already words as "we can't
     ship this to your country" — accurate, and no extra handling needed. */
  mug: [
    { id: 'mug-11oz', label: '11oz Photo Mug', sku: 'GLOBAL-MUG-W', aspect: 228.6 / 94.83, sizing: 'fillPrintArea', printArea: 'default', attributes: {}, minCopies: 1, wrap: { widthMm: 228.6, heightMm: 94.83, safeMm: 4 }, asset: { format: 'jpeg', dpi: 300 } },
    // UK-only, hidden — see above. Verified SKUs, kept resolvable, not sold.
    ...[
      ['BLA',  'Black'],
      ['RED',  'Red'],
      ['YLW',  'Yellow'],
      ...
      id: `mug-11oz-${code.toLowerCase()}`,
      label: `11oz Coloured Mug (${name})`,
      sku: `H-MUG-11OZ-${code}`,
      aspect: 190 / 100,
      ...
      attributes: {},
      minCopies: 1,
      wrap: { widthMm: 190, heightMm: 100, safeMm: 4 },
      asset: { format: 'jpeg', dpi: 300 },
      hidden: true,
    })),
  ],
...
export const COMPANION_INSERT_FOR = new Set([
  ...
  'book', 'calendar',
  // The gift products need this MORE than anything else here, not less. A
  // magnet, sticker or ornament is single-sided with no back cover and no room
  // for a URL, so the card in the box is the only thing that tells whoever
  // receives it where to go. A mug does carry the URL in its wrap artwork, but
  // it keeps the card too: the wrap line is small, it can end up facing away,
  // and these ship as their own UK parcel regardless.
  // ...except the ornament, which is the one that does NOT take a card: it is
  // fulfilled by Printify, and Printify has no insert mechanism at all. Its URL
  // is printed on the ornament's own back panel instead.
  'magnet', 'sticker', 'mug',
]);
...
/* How many of a variant an order must contain.

   Some products cannot be sold singly. A magnet costs pennies to print, but it
   still has to be made in the UK and posted, and it still draws the ~$3.75 of
   branded inserts that every shipment carries and no quote mentions — so one
   magnet on its own is underwater before anything else happens. Those variants
   carry `minCopies`, which is really a pack size: the shop starts the quantity
   there and cannot step below it, so the customer never meets a rejection, they
   just buy a set of three the way anyone buying fridge magnets would.

   Enforced on the SERVER as well as in the UI, because the UI is not trusted
   with SKUs or prices and should not be trusted with this either.

   Set the real numbers from
     node scripts/price-list.mjs --type=magnet,sticker,ornament,mug --copies=N
   whose "min qty" column is exactly how many of an item an order needs before
   it carries its own inserts. */
export function minCopiesFor(variant) {
  const n = parseInt(variant && variant.minCopies, 10);
  return Number.isFinite(n) && n > 1 ? n : 1;
}

/* Clamp a requested quantity into [minCopies, 99]. Used by the quote, cart and
   checkout paths so all three agree; a client asking for fewer than the pack
   size is raised to it rather than rejected. */
export function normalizeCopies(variant, copies) {
  const asked = Math.max(1, parseInt(copies, 10) || 1);
  return Math.min(99, Math.max(minCopiesFor(variant), asked));
}
...
export const TYPE_MARKUPS = {
  ...
  calendar: 1.5,
  boardbook: 1.5,
  // The gift products are the most shipping-dominated things in the catalogue:
  // cheap goods made in the UK and posted across an ocean. A sticker costs
  // under a pound to print and several dollars to send, so the formula above
  // (1 + 0.40 + 0.25 x shipping/goods) lands much higher than it does for a
  // framed print — the multiplier looks alarming only because the goods figure
  // it multiplies is tiny.
  //
  // These four are ESTIMATES from Prodigi's published "from" prices, not from a
  // real quote, because none of these SKUs has been quoted yet. Run
  //   node scripts/price-list.mjs --type=magnet,sticker,ornament,mug --copies=3
  // and tune them here (or without a deploy via PRINT_MARKUP_BY_TYPE).
  magnet: 2.4,
  sticker: 2.9,
  ornament: 1.7,
  mug: 1.9,
