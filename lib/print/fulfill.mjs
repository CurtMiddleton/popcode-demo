...
/* What the customer paid US, excluding sales tax.

   `amount_total` is the whole charge — goods, shipping, discounts AND tax. Tax
   is collected on the state's behalf and remitted, so it is not revenue, and
   total_charged_minor feeds the "Revenue" figure in analytics. Subtracting it
   keeps that number meaning what it says.

   It still has to come off the Stripe session rather than the quote, because
   promotion codes are enabled (allow_promotion_codes) and a discount only
   exists here — that reconciliation is the whole reason this value is read
   back at all.

   Tax that WAS collected is not recorded here; Stripe's own Tax → Transactions
   is the record for filing, and duplicating it in our table would give two
   answers to one question. */
export function settledAmountMinor(session) {
  if (!session || typeof session.amount_total !== 'number') return null;
  const tax = Number(session.total_details?.amount_tax) || 0;
  return Math.max(0, session.amount_total - tax);
}
...
export async function fulfillSession({ admin, orders, amountTotalMinor = null, onError = null }) {
  ...
  await settlePopcodeCredits({ admin, orders, onError });
  return results;
}

// "Popcodes on anything you buy from the Popcode Shop are always free and
// unlimited". A Popcode exists before anyone buys a print of it, so it cannot be
// exempted when it is made — a paid order refunds it instead, which leaves the
// buyer exactly where the page says: those ones never counted.
//
// Runs here rather than in either endpoint so the success page and the webhook
// both get it from one place. The RPC does the claiming, so calling it twice
// grants once. Best-effort on purpose: a print order that is paid and submitted
// must never fail because of a credit refund.
async function settlePopcodeCredits({ admin, orders, onError = null }) {
  for (const order of orders) {
    if (!order || !order.id) continue;
    try {
      const { error } = await admin.rpc('popcode_settle_print_order', { order_id: order.id });
      if (error) throw new Error(error.message);
    } catch (err) {
      const wrapped = new Error(`Popcode credit refund failed for order ${order.id}: ${err.message}`);
      if (onError) onError(wrapped, { order_id: order.id });
