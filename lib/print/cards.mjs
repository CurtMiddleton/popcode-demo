...
const zone = (id, role, dflt, geom) => ({
  ...
  track: 0, upper: false,
  // Leading and finish are per-zone: display lines want tight leading, and a
  // foil finish applies to one line (a whole card in foil looks like a novelty).
  lead: null, finish: 'flat',
...
// Great Vibes was the tell. It is the most-used free script on the internet --
// every Etsy wedding template, every canva invite -- so it reads as a stock
// template no matter how good the photo is. Ballet is the same register with far
// less mileage. Bodoni Moda is the high-contrast didone that does most of the
// work in premium stationery (Vogue, Minted); Fraunces is the modern editorial
// serif. All SIL OFL, so all safe for a POD configurator (see the licence note).
export const FONTS = {
  script:    { label: 'Script',    css: "'Ballet', 'Snell Roundhand', cursive",       google: 'Ballet' },
  didone:    { label: 'Didone',    css: "'Bodoni Moda', 'Didot', serif",              google: 'Bodoni+Moda:ital,opsz,wght@0,6..96,400;0,6..96,600;1,6..96,400' },
  editorial: { label: 'Editorial', css: "'Fraunces', Georgia, serif",                 google: 'Fraunces:ital,opsz,wght@0,9..144,300;0,9..144,500;1,9..144,300' },
  serif:     { label: 'Serif',     css: "'Playfair Display', Georgia, serif",         google: 'Playfair+Display:ital,wght@0,400;0,600;1,400' },
  cormorant: { label: 'Garamond',  css: "'Cormorant Garamond', Garamond, serif",      google: 'Cormorant+Garamond:ital,wght@0,300;0,400;0,600;1,300;1,400' },
  cinzel:    { label: 'Engraved',  css: "'Cinzel', 'Trajan Pro', serif",              google: 'Cinzel:wght@400;600' },
  grotesk:   { label: 'Grotesk',   css: "'Jost', 'Futura', sans-serif",               google: 'Jost:wght@300;400;500' },
  sans:      { label: 'Sans',      css: "'Inter', sans-serif",                        google: null },
};

// ── The measurements that separate a $1 card from a $4 one ─────────────────
// Arbitrary y-values were the second tell. Premium stationery sits on a system:
// one optical margin, a modular size scale, and leading that TIGHTENS as type
// grows. The old code used lead 1.15 for everything, which makes a 13% display
// line look loose and slack -- display wants ~1.02, small text wants ~1.4.
export const TYPE = {
  margin: 0.075,                              // fraction of card WIDTH, all sides
  measure: 0.85,                              // default content width (1 - 2*margin)
  display: { xl: 0.150, lg: 0.118, md: 0.092, sm: 0.072 },
  body:    { lg: 0.034, md: 0.028, sm: 0.023 },
  lead:    { display: 1.02, mid: 1.18, body: 1.40 },
  // Tracking: display lowercase wants slightly NEGATIVE (it sets loose at size);
  // caps always want generous positive or they clot together.
  track:   { display: -0.015, caps: 0.16, capsWide: 0.28 },
};

// Foil is the single biggest perceived-value lever available in pure CSS, and
// it is exactly what Shutterfly upcharges for ("AVAILABLE IN FOIL"). A metallic
// is a GRADIENT, never a flat gold -- the bands are what read as reflection.
export const FINISHES = {
  flat:   { label: 'Flat' },
  gold:   { label: 'Gold Foil',   gradient: 'linear-gradient(100deg,#8a6a2f 0%,#d9b45e 17%,#f7e9b0 33%,#c99b3f 50%,#f2dc98 67%,#b98c33 84%,#7d5f28 100%)' },
  rose:   { label: 'Rose Foil',   gradient: 'linear-gradient(100deg,#8c5245 0%,#d69b88 17%,#f6ddd2 33%,#c07f6b 50%,#efd0c2 67%,#b0705c 84%,#7d4739 100%)' },
  silver: { label: 'Silver Foil', gradient: 'linear-gradient(100deg,#6e727a 0%,#b8bec7 17%,#eef1f5 33%,#9aa1ab 50%,#e2e6ec 67%,#8b929c 84%,#63676e 100%)' },
};
...
export const CARD_TEMPLATES = [
  // ── Full-bleed: photo edge to edge, type laid on top ──
  // `scrim` values are deliberately LOW. A heavy dark smear under the type is
  // the classic amateur tell; the renderer draws an EASED multi-stop gradient so
  // there is no visible onset edge, and the type carries its own tiny shadow.
  {
    id: 'bleed-script',
    name: 'Winter Script',
    formatId: 'flat-5x7', orientation: 'portrait',
    tags: ['christmas', 'holiday', 'baby', 'pet', 'newlywed'],
    ground: 'bleed', ornament: 'snow', scrim: 0.26,
    colorways: ['cream', 'midnight', 'sage', 'burgundy'],
    photos: [{ id: 'main', x: 0, y: 0, w: 1, h: 1, fit: 'cover' }],
    popcodeSlot: 'main',
    text: [
      zone('headline',  'headline',  'Happy Holidays',       { x: 0.5, y: 0.740, size: 0.118, maxW: 0.85, color: 'onPhoto', font: 'script', lead: 1.00, lines: 1 }),
      zone('signature', 'signature', 'The Middleton Family', { x: 0.5, y: 0.880, size: 0.023, maxW: 0.78, color: 'onPhoto', font: 'grotesk', track: 0.28, upper: true, lead: 1.40 }),
    ],
  },
  {
    id: 'bleed-stacked',
    ...
    ground: 'bleed', ornament: 'none', scrim: 0.30,
    colorways: ['cream', 'midnight', 'burgundy', 'blush'],
    photos: [{ id: 'main', x: 0, y: 0, w: 1, h: 1, fit: 'cover' }],
    popcodeSlot: 'main',
    text: [
      zone('headline',  'headline',  'MERRY',                { x: 0.5, y: 0.700, size: 0.118, maxW: 0.85, color: 'onPhoto', font: 'didone', track: 0.16, upper: true, lead: 1.02, lines: 1 }),
      zone('subhead',   'subhead',   'CHRISTMAS',            { x: 0.5, y: 0.806, size: 0.072, maxW: 0.85, color: 'onPhoto', font: 'didone', track: 0.28, upper: true, lead: 1.02, lines: 1 }),
      zone('signature', 'signature', 'the Middletons',       { x: 0.5, y: 0.892, size: 0.023, maxW: 0.78, color: 'onPhoto', font: 'grotesk', track: 0.22, upper: true, lead: 1.40 }),
    ],
  },
  {
    id: 'bleed-corner',
    ...
    ground: 'bleed', ornament: 'none', scrim: 0.22,
    colorways: ['cream', 'sage', 'midnight'],
    photos: [{ id: 'main', x: 0, y: 0, w: 1, h: 1, fit: 'cover' }],
    popcodeSlot: 'main',
    text: [
      zone('headline',  'headline',  'joy',                  { x: 0.075, y: 0.712, size: 0.150, maxW: 0.60, color: 'onPhoto', font: 'script', align: 'left', lead: 1.00, lines: 1 }),
      zone('signature', 'signature', 'the Middletons 2026',  { x: 0.075, y: 0.902, size: 0.023, maxW: 0.62, color: 'onPhoto', font: 'grotesk', align: 'left', track: 0.28, upper: true, lead: 1.40 }),
    ],
  },

  // ── Inset: photo framed on a coloured ground ──
  // The photo carries a hairline and a small corner radius; a bare hard-edged
  // rectangle on a flat ground is what makes an inset card look like a Word doc.
  {
    id: 'inset-arch',
    name: 'Arched Branches',
    formatId: 'flat-5x7', orientation: 'portrait',
    tags: ['christmas', 'holiday', 'newlywed', 'baby'],
    ground: 'inset', ornament: 'branches', mat: true,
    colorways: ['cream', 'sage', 'blush', 'burgundy'],
    photos: [{ id: 'main', x: 0.15, y: 0.205, w: 0.70, h: 0.545, fit: 'cover', arch: true }],
    popcodeSlot: 'main',
    text: [
      zone('headline',  'headline',  'HAPPY HOLIDAYS',       { x: 0.5, y: 0.115, size: 0.034, maxW: 0.78, color: 'ink', font: 'cinzel', track: 0.28, upper: true, lead: 1.40, finish: 'gold' }),
      zone('signature', 'signature', 'From, the Middletons', { x: 0.5, y: 0.828, size: 0.023, maxW: 0.78, color: 'accent', font: 'grotesk', track: 0.16, lead: 1.40 }),
    ],
  },
  {
    id: 'inset-signature',
    ...
    ground: 'inset', ornament: 'none', mat: true,
    colorways: ['cream', 'sage', 'burgundy', 'midnight'],
    photos: [{ id: 'main', x: 0.22, y: 0.300, w: 0.56, h: 0.400, fit: 'cover' }],
    popcodeSlot: 'main',
    text: [
      zone('headline',  'headline',  'HAPPY',                { x: 0.5, y: 0.120, size: 0.092, maxW: 0.85, color: 'ink', font: 'didone', track: 0.16, upper: true, lead: 1.02, lines: 1 }),
      zone('subhead',   'subhead',   'the Middletons, 2026', { x: 0.5, y: 0.222, size: 0.046, maxW: 0.72, color: 'ink', font: 'script', lead: 1.10, lines: 1 }),
      zone('signature', 'signature', 'HOLIDAYS',             { x: 0.5, y: 0.762, size: 0.092, maxW: 0.85, color: 'ink', font: 'didone', track: 0.16, upper: true, lead: 1.02, lines: 1 }),
    ],
  },
  {
    id: 'inset-frame',
    ...
    photos: [{ id: 'main', x: 0.125, y: 0.150, w: 0.75, h: 0.580, fit: 'cover' }],
    popcodeSlot: 'main',
    text: [
      zone('headline',  'headline',  'Season\u2019s Greetings', { x: 0.5, y: 0.798, size: 0.072, maxW: 0.80, color: 'ink', font: 'cormorant', lead: 1.02, lines: 1, finish: 'gold' }),
      zone('signature', 'signature', 'THE MIDDLETONS',       { x: 0.5, y: 0.892, size: 0.023, maxW: 0.78, color: 'accent', font: 'grotesk', track: 0.28, upper: true, lead: 1.40 }),
    ],
  },
  ...
  // ── Collage ──
  {
    ...
    ground: 'collage', ornament: 'none', mat: true,
    colorways: ['sage', 'cream', 'midnight', 'burgundy'],
    photos: [
      { id: 'p1', x: 0.075, y: 0.075, w: 0.415, h: 0.275, fit: 'cover' },
      { id: 'p2', x: 0.510, y: 0.075, w: 0.415, h: 0.385, fit: 'cover' },
      { id: 'p3', x: 0.075, y: 0.370, w: 0.415, h: 0.275, fit: 'cover' },
      { id: 'p4', x: 0.510, y: 0.480, w: 0.415, h: 0.275, fit: 'cover' },
    ],
    popcodeSlot: 'p1',
    text: [
      zone('headline',  'headline',  'ENJOY EVERY MOMENT',   { x: 0.5, y: 0.800, size: 0.040, maxW: 0.85, color: 'ink', font: 'editorial', track: 0.16, upper: true, lead: 1.18, lines: 2 }),
      zone('signature', 'signature', 'the Middletons 2026',  { x: 0.5, y: 0.900, size: 0.023, maxW: 0.80, color: 'accent', font: 'grotesk', track: 0.22, upper: true, lead: 1.40 }),
    ],
  },
  {
    id: 'inset-birth',
    ...
    photos: [{ id: 'main', x: 0.10, y: 0.10, w: 0.80, h: 0.585, fit: 'cover' }],
    popcodeSlot: 'main',
    text: [
      zone('subhead',   'subhead',   'INTRODUCING',          { x: 0.5, y: 0.748, size: 0.023, maxW: 0.80, color: 'accent', font: 'grotesk', track: 0.28, upper: true, lead: 1.40 }),
      zone('headline',  'headline',  'Adelaide Rose',        { x: 0.5, y: 0.796, size: 0.072, maxW: 0.85, color: 'ink', font: 'cormorant', lead: 1.02, lines: 1 }),
      zone('signature', 'signature', 'March 4, 2026',        { x: 0.5, y: 0.898, size: 0.023, maxW: 0.82, color: 'accent', font: 'grotesk', track: 0.22, upper: true, lead: 1.40 }),
    ],
  },
];
