...
// Print-asset pixel dimensions for a format at full bleed.
export function cardAssetSize(formatId, orientation = 'portrait') {
  const f = CARD_FORMATS[formatId];
  if (!f) return null;
  const land = orientation === 'landscape';
  const wIn = (land ? f.hIn : f.wIn) + f.bleedIn * 2;
  const hIn = (land ? f.wIn : f.hIn) + f.bleedIn * 2;
  return { w: Math.round(wIn * f.dpi), h: Math.round(hIn * f.dpi), dpi: f.dpi };
}
...
export const TEXT_ROLES = {
  headline:  { label: 'Greeting',   maxChars: 28,  hint: 'Happy Holidays' },
  subhead:   { label: 'Second line',maxChars: 40,  hint: 'Wishing you joy' },
  signature: { label: 'Signature',  maxChars: 44,  hint: 'Love, the Middletons' },
  year:      { label: 'Year',       maxChars: 9,   hint: '2026' },
  message:   { label: 'Message',    maxChars: 240, hint: 'A note on the back' },
};
