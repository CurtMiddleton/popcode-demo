...
export async function quoteCart({ lines, address, shippingMethod, markup, getProvider, attempts = 3 }) {
  ...
  /* Checkout shows two lines that must add up to exactly what is charged.
     Shipping is shown AT COST, rounded to the whole unit, and everything else
     sits in the printing line. Showing a marked-up figure beside the word
     "shipping" would misdescribe it — the point of breaking it out is that the
     customer can see what the carrier actually costs. The margin is unchanged
     either way; only the presentation differs. */
  const shippingMinor = Math.min(
    Math.round(shippingCostMinor / 100) * 100,
    Math.max(0, totalMinor - 100),
  );
  ...
  return {
    ...
    currency,
    shippingMinor,
    printingMinor: totalMinor - shippingMinor,
  };
}
