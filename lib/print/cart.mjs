...
import {
  findVariant, priceFromQuote, providerFor,
  COMPANION_CARD, companionCardCollectionIds, companionCardPaths, companionCardAssets,
} from './catalog.mjs';
...
/**
 * Append the companion postcard lines an order earns.
 *
 * Called from normalizeLines so pricing and checkout get it from the SAME
 * place — if only one of them added the card, the price shown would not be the
 * price charged. This is the "can never drift" rule at the top of this file.
 *
 * The card is server-authored: the client never sends it and never names its
 * asset URLs, which are derived from the slug. At quote time no slug is needed
 * (Prodigi prices by SKU), so `slugFor` is omitted and the line carries no
 * assets — enough to price the right number of cards.
 *
 * @param {Array} lines normalized client lines
 * @param {object} [opts]
 * @param {Record<string,string>} [opts.slugFor] collectionId -> slug, checkout only
 * @param {string} [opts.assetPrefix] public storage prefix, checkout only
 */
export function withCompanionCards(lines, { slugFor = null, assetPrefix = null } = {}) {
  if (!COMPANION_CARD.enabled) return lines;
  const ids = companionCardCollectionIds(lines);
  if (!ids.length) return lines;

  const cards = ids.map((collectionId) => {
    const line = {
      id: `companion:${collectionId}`,
      collectionId,
      productType: 'companion',
      variantId: COMPANION_CARD.id,
      variant: COMPANION_CARD,
      copies: 1,
      pageCount: null,
      assetUrls: [],
      title: COMPANION_CARD.label,
      provider: providerFor('companion'),
    };
    if (slugFor) {
      const slug = slugFor[collectionId];
      // No slug means no URL to print, which would be a blank card — refuse
      // rather than ship one.
      if (!slug) throw new CartError('Could not prepare the companion card for this order', { status: 500 });
      const paths = companionCardPaths(slug);
      const urls = {};
      for (const { face } of COMPANION_CARD.faces) urls[face] = `${assetPrefix}${paths[face]}`;
      line.assetUrls = companionCardAssets(urls);
    }
    return line;
  });

  return lines.concat(cards);
}
