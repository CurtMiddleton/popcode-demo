...
export async function quote({ lines = null, variant, copies = 1, address = null, shippingMethod }) {
  ...
  if (haveAddress) {
    ...
    if (!r.ok) {
      ...
      // "We don't ship that there" — but only for the statuses that actually mean
      // it. A 401/403 is our token, not the route. See UNSERVABLE_STATUSES.
      err.unservable = UNSERVABLE_STATUSES.has(r.status);
      throw err;
    }
    ...
    if (typeof shipMinor === 'number') { totalMinor += shipMinor; shippingMinor += shipMinor; }
  }
  // Reported apart from the item cost so checkout can show a shipping line that
  // is right for a mixed cart, not just a Prodigi-only one.
  return { totalMinor, shippingMinor, currency };
}
