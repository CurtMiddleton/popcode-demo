...
// The SKUs in a quote's items[], for an error message that names the culprit
// instead of leaving someone to guess which line of a cart was wrong.
function variantSkusIn(items) {
  return [...new Set((items || []).map((i) => i && i.sku).filter(Boolean))].join(', ') || 'unknown';
}
...
export async function quote({ lines = null, variant, copies, pageCount = null, destinationCountryCode, shippingMethod }) {
  ...
  if (!resp.ok) {
    ...
    // SkuNotFound is a 404, but it is OUR catalogue being wrong, not a route
    // this customer can't be served. Left in the unservable bucket it reads to
    // the customer as "we can't ship that to your country" and to us as normal
    // behaviour — so a SKU typo would sell nothing and report nothing, which is
    // precisely how it would hide. Deterministic, so still no retry; just
    // reported and worded as our problem rather than theirs.
    if (/SkuNotFound/i.test(text)) {
      err.unservable = false;
      err.skuNotFound = true;
      // Not unservable, so it isn't dressed up as a destination problem — but
      // still deterministic, so `noRetry` stops the callers' retry loops from
      // asking twice more for the same certain answer.
      err.noRetry = true;
      err.message = `Prodigi does not recognise SKU "${variantSkusIn(items)}" — check it with `
        + `scripts/verify-prodigi-sku.mjs (${resp.status})`;
    }
    throw err;
  }
