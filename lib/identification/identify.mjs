...
//   2. ClipProvider.identify(creator_id, frame): embed + scoped
...
import { ClipProvider } from './provider.mjs';
...
export async function identifyByHandle(db, handle, frame, { threshold, supabaseUrl } = {}) {
  ...
  // 1. Resolve the creator (case-insensitive handle) and enforce the per-handle
  //    cutover flag — the new experience is live only for enabled creators.
  const { data: creator, error: cErr } = await db
    .from('creators').select('id, new_identification_enabled').ilike('handle', handle).maybeSingle();
  if (cErr) throw new Error('creator lookup failed: ' + cErr.message);
  if (!creator) return { matched: false, reason: 'unknown_handle' };
  if (!creator.new_identification_enabled) return { matched: false, reason: 'handle_not_enabled' };

  // 2. Scoped search — get the top candidates so we can log the full picture.
  const provider = new ClipProvider(db, threshold != null ? { threshold } : {});
  ...
  const { data: imgs } = await db
    .from('pop_images')
    .select('id, target_ref, video_url, audio_first, media_type, audio_url, transcript, image_url, embedding')
    .eq('collection_id', top.collectionId);
  const images = (imgs || [])
    .map(i => ({
      id: i.id, target_ref: i.target_ref,
      media_type: i.media_type || 'video',
      video_url: i.video_url, audio_url: i.audio_url, transcript: i.transcript,
      image_url: i.image_url, audio_first: i.audio_first,
      // 512-float L2-normalized CLIP vector (pgvector text form). Lets the client
      // cache this book and match its other pages locally — no further server call.
