...
//
// Start with CLIP embedding + pgvector cosine search (ClipProvider below).
// Embedding is on-device (transformers.js); see lib/identification/embed.mjs.
// A feature-descriptor matcher or a hosted recognition API can replace it later
// WITHOUT touching callers — same identify() contract.
//
...
export class ClipProvider {
   ...
   *        0.60 was tuned for the OLD Replicate 768-dim model. The on-device
   *        ViT-B/32 (q8, 512-dim) has a different score distribution — RE-TUNE
   *        from Phase 4 identify_events data (override live via IDENTIFY_THRESHOLD).
   */
