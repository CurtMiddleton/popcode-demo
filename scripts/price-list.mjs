  ...
  for (const variant of PRODUCTS[type]) {
    ...
    try {
      const q = await provider.quote({
        variant, copies: 1, pageCount: pages,
...
if (CSV) {
  ...
  for (const r of rows) {
    if (r.type !== lastType) {
      lastType = r.type;
      console.log(`\n${r.type.toUpperCase()}`);
      console.log(`  ${pad('variant', 22)}${padL('product', 10)}${padL('shipping', 10)}${padL('cost', 10)}${padL('price', 10)}${padL('margin', 10)}  note`);
      console.log('  ' + '─'.repeat(84));
    }
    if (r.error) { console.log(`  ${pad(r.id, 22)}${padL('—', 10)}${padL('—', 10)}${padL('—', 10)}${padL('—', 10)}${padL('—', 10)}  ⚠ ${r.error}`); continue; }
    ...
    console.log(`  ${pad(r.id, 22)}${padL(money(r.itemMinor, r.currency), 10)}${padL(money(r.shipMinor, r.currency), 10)}${padL(money(r.costMinor, r.currency), 10)}${padL(money(r.priceMinor, r.currency), 10)}${padL(money(r.marginMinor, r.currency), 10)}  ${note}`);
  }
