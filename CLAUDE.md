...
### 2026-08-03 — Shop buildout (8x8 book, calendar maker, frames, Unsplash samples) + home page redesign with interactive scroll hero

**Branch `claude/shop-options-unsplash-bim181`. Shop work merged to `main` incrementally (live in prod, last merge `46b7c2b`). The home-page redesign is ON THE BRANCH ONLY — not merged, awaiting the user's art direction on the Vercel branch preview.**

#### Shop expansion (all merged to prod)
- **8x8 square Prodigi photo book** added alongside A4 Landscape (`BOOK_SIZES` in book.html with `--book-ar`; intro mockup swaps `book`/`book-square` image when the size chip changes, square capped at max-width 71%).
- **Calendar maker (`public/calendar.html`)** — 12-month undated A4 landscape wall calendar, Popsa-style design from the Prodigi template PDF: big month name, columnar week grid with vertical rules (grid pushed low, `gridTop=440` on a 1400x993 canvas), gray month number, dates-on-top connected to photo-below as one sheet. Same 6 font choices as the book (Filson kept + default). Popcode editor ported from book.html; saving compiles a `.mind` + writes `collection_items` (popcoded calendars appear in My Popcodes); popcode URL (slug) is shown and editable pre-save, locked after. Real scan badge `.pf-badge` at print scale (PXH*0.09) — replaced the wrong purple "Video" pill.
- **Catalog growth (`lib/print/catalog.mjs` + client mirror in order.html):** framed prints GLOBAL-CFP (black/white/natural frame colors), mounted framed GLOBAL-CFPM, framed canvas GLOBAL-FRA-SLIMCAN, acrylic GLOBAL-MOU-ACRY, tile colors (black/white). Color variants: black keeps the bare variant id (backward compat), white/natural get `-color` suffix; `orderVariantId()` composes them. Shop order: Book, Calendar, Framed Prints, Tiles, Canvas, Framed Canvas, Acrylic, Prints. **NONE of the new SKUs sandbox-quote-verified yet — verify each via a Prodigi quote before real orders.**
- **Photo tiles: square corners** (removed roundRectPath for tiles — the real PHOTIL-FRA product has square edges).
- **Sample imagery (`public/unsplash-samples.js`):** pool from the user's 50 hand-picked Drive photos (Drive direct download 401s — resolved by extracting Unsplash photo IDs from filenames and hotlinking images.unsplash.com). **12 Unsplash+ (plus.unsplash.com) photos REMOVED — they render tiled watermarks when hotlinked** (thumbnail-size verification missed it; the user's kitten screenshot caught it). To restore: self-host originals under /assets/samples/ (documented in the file header). Leopard fully retired (was the old sample): fixed the sliver on the shop book card + the template flash on load. Product sizes normalized between shop cards and detail pages (crop canvas to product bounding box so CSS sizes the product, not padding).
- **My Popcodes cards:** product kind pill (Book, Calendar, Framed Print...) now renders ABOVE the title styled like the gray COLLECTION pill but purple (`.pop-pill.product`, gradient bg). Old inline `.kind-pill` removed.
- Build-progress copy de-jargoned + emojis removed sitewide (per user).

#### Home page redesign (BRANCH ONLY — the big one)
User asked how to convey what Popcode is on the home page; direction settled: **one home page for popcode.app (logged-in and out states), then forward popcodeapp.com to it**, replacing the separate marketing site.

- **`public/index.html` fully rebuilt** as a marketing landing while keeping the viewer function (code entry + Go JS, sign-in link, beta-feedback, Sentry). NOTE: line 34 is an 80KB base64 CooperBT @font-face — edit the file by python-splicing around it, never rewrite whole-file by hand.
- **Sections:** hero (below) → "Three steps to magic" (gradient CooperBT numbers, copy borrowed from popcodeapp.com) → "Two ways to Popcode" (Create an experience = subscription avenue vs Shop scannable keepsakes = pay-per-item) → "What will you bring to life?" 6 use-case cards using the marketing site's one-liners (Grandma's albums / wedding wall / galleries & exhibits / travel / In Memoriam "Their voice. Their smile. Forever near." / dark "...and anything you can print" card) → closing band "Some pictures deserve to be heard." → footer. Headline: **"Every picture has a story. Now it can tell it."** — "picture" (not photo) is deliberate, covers paintings/posters/pages per user.
- **Interactive hero demo** (iterated 4x with user): a framed print + a phone mockup whose screen is a **simulated camera view** (clone img transformed so the scene under the screen center renders on screen, magnifier-style, z=1.15). Phone starts far from the print showing a **miniature of the real app home screen** (logo, tagline, Tap to Scan pill). Flow: Tap to Scan → camera/scanning mode with "Point at a picture" hint pill → move over print → overlap>0.55 = found (pulse rings) → 700ms → playing (Ken Burns zoom on the still + "Dad tells the story" chip). **`DEMO_VIDEO_URL` const in the script swaps the Ken Burns for a real mp4 when the user picks a clip** (do NOT use their family videos without explicit choice).
- **Scroll-driven is the DEFAULT** (user's final call): `.hero-sticky-wrap` 210vh + sticky hero; scroll progress k drives the phone (scan starts k>0.06, arrives at k=0.72 for a dwell phase, resets to home screen at k<=0.02); bouncing chevron cue fades after k>0.15. **`?hero=drag`** switches to the draggable version (pointer drag + auto-demo glide after 1.4s). Mobile caveat flagged to user: hero content is taller than the viewport so the pin is partial (hero drifts while animating) — full freeze would need slimming the hero (e.g. code entry below the pinned section). Awaiting their verdict from a real iPhone.
- **Logged-in touch:** detects a Supabase session WITHOUT loading supabase-js — `Object.keys(localStorage).some(k => /^sb-.*-auth-token$/.test(k))` — and swaps the sign-in line for "Welcome back — go to My Popcodes".
- Images: verified-working Unsplash pool photos + one new curl-verified gallery corridor (photo-1572947650440). OG description updated.

#### Gotchas / lessons this session
- **Hotlinked Unsplash+ premium photos watermark at render size** even though small thumbnails look clean. Verify samples at display size, or self-host.
- **Headless chromium in the sandbox cannot reach images.unsplash.com** (no proxy for the browser; curl works via HTTPS_PROXY). Pattern: curl the images to scratchpad, then `page.route` fulfill them from disk.
- The local test http.server MUST serve `public/` — a server started from repo root 404s everything; check the real cwd with `readlink -f /proc/<pid>/cwd`. `pkill -f "http.server 8099"` can kill the calling shell's process group (exit 144) — start servers with `setsid ... < /dev/null &` instead.
- **popcode_badge.svg does NOT exist in the repo** (only in session-notes lore / the user's Dropbox); `public/assets/popcode_icon.svg` is the only badge asset. It's the authentic print badge, but reads dark below ~36px.
- The marketing site source (`marketing/index.html`) has the best copy lines — "The wish, not just the cake.", "Their voice. Their smile. Forever near." — mine it before writing new copy.
- Editing rebuilt index.html: line 34 is one giant base64 font line; python splice (keep lines 1-34, replace tail) works cleanly. Always `node --check` the extracted inline script after edits.

#### Open / next
- **User art direction on the home page** → then merge to prod + set up the popcodeapp.com → popcode.app forward (Squarespace/Vercel) + decide how much the logged-in home state changes.
- Pick a real mp4 for `DEMO_VIDEO_URL` (biggest hero upgrade).
- "Subscription plans for creators" line is a placeholder — pricing not defined.
- Verify all new Prodigi SKUs via sandbox quotes before real orders (square book, CFPM, FRA-SLIMCAN, MOU-ACRY, white/natural color casing).
- Optionally self-host the 12 premium Unsplash photos to restore them to the sample pool.
